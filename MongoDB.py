...
def date_sort(db, first_date, second_date):
    ...
    if fd > sd:
        ...
        sorted_database = db.collection.find({'Дата':{'$lte': sd, '$gte': fd}}).sort('Дата', 1)
...
def read_data(csv_file, db):
    ...
    db.collection.insert_many(artists_list)


def delete_data(db):
    """
    Удалиль коллекцию
    Документация: https://docs.mongodb.com/manual/reference/method/db.collection.drop/index.html
    """
...
def find_cheapest(db):
    ...
    sorted_database = db.collection.find().sort('Цена', 1)
    for i in sorted_database:
        #print(i)  # Для сортировки по цене уберите (#) перед print
        pass
...
def find_by_name(name, db):
    ...
    sorted_database = db.collection.find({'Исполнитель': regex}).sort('Цена', 1)
...
if __name__ == '__main__':
    ...
    find_by_name(input('Введите название группы или имя исполнителя:'), music_artists_db)
